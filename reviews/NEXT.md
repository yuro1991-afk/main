...
`c353a04`. Shipped this interval: `faceswap-honesty-located-paths`
Located Path/Models afterApply. Leftover `next` stays alone. Do not invent leftovers.
...
| next afterApply to wire | `dronehive-work-order-registry-cli` (work_order.py CLI fallback only) |
...
Next afterApply to wire is `dronehive-work-order-registry-cli`.
