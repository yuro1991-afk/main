...
Keep-busy tick **2026-09-15T01:41Z**: 40 IDLE + this run RUNNING.
...
`601542b`. Shipped this interval: `dronehive-seed-work-order-doc-cd`
seed swarm-entry cd afterApply. Leftover `next` stays alone. Do not invent leftovers.
...
| next afterApply to wire | `dronehive-spec-relative` (DroneHive.spec Analysis only) |
...
(standalone `cd .` line; G: project cd is gone).
Next afterApply to wire is `dronehive-spec-relative`.
...
then unused `dronehive-spec-relative` (DroneHive.spec Analysis only),
