...
Keep-busy tick **2026-09-14T22:54Z**: 40 IDLE + this run RUNNING.
...
CI green on `7f43471`. Leftover hunt is exhausted for vanilla+stacked
`git apply --check` (no `-U0`). Do not invent leftovers. Do not
recatalog CHANNEL_META / wraps / sandwiched JSON.
Shipped this tick: leftover next `gub-superbrain-probe` firstCommands
and packets refuse the probe (Yuri: no more Superbrain). Do not run
