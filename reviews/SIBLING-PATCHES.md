# Sibling patch catalog (verified 2026-09-14T19:45Z)
...
- dronehive: do not rebase dronehive#2 until #1 `python-smoke` is green. Ubuntu smoke, portable-paths, script-host-roots, runtime-host-paths, config-load-overlay, icons-manifest-relative, and app-links-host-paths stack on the unicode patch. Apply `dronehive-runtime-host-paths`, `dronehive-config-load-overlay`, and `dronehive-app-links-host-paths` **after** portable-paths (they are independent of each other). `dronehive-icons-manifest-relative` is independent of portable-paths. Do not edit `drone/pro/tool_agent.py` in the path patches.
