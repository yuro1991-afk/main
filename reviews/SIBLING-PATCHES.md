...
- dronehive: do not rebase dronehive#2 until #1 `python-smoke` is green. Ubuntu smoke, portable-paths, script-host-roots, runtime-host-paths, config-load-overlay, icons-manifest-relative, app-links-host-paths, hive-docstring-honesty, work-order-doc-honesty, and seed-work-order-doc-honesty stack on the unicode patch. Apply `dronehive-runtime-host-paths`, `dronehive-config-load-overlay`, and `dronehive-app-links-host-paths` **after** portable-paths (they are independent of each other). `dronehive-icons-manifest-relative`, `dronehive-hive-docstring-honesty`, `dronehive-work-order-doc-honesty`, and `dronehive-seed-work-order-doc-honesty` are independent of portable-paths. Do not edit `drone/pro/tool_agent.py` in the path patches.
