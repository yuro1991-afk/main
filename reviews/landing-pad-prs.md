...
| [#9](https://github.com/yuro1991-afk/main/pull/9) | MERGEABLE + CI green | **merge after #8**. List-only catalog (162, re-proof 2026-09-14T23:16Z). `playbooks` and `patches` default to compact (`nextApply: dronehive-unicode-ci`). `patches --prove` and `--prove-after-apply` use throwaways (never write or reset `/tmp/siblings`). Independent of #8 (branched from merged #3). `siblings.json` on #9 already lists PRs 2–10. Expect a residual array conflict vs #8 (2–8); take #9’s union. CI on #9 is checkout@v5 / setup-node@v5 (keep it; #8 still has v4). |
