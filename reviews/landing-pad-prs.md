...
| [#9](https://github.com/yuro1991-afk/main/pull/9) | MERGEABLE + CI green | **merge after #8**. List-only catalog (162, re-proof 2026-09-14T23:16Z). `playbooks` defaults to `--check` (`nextApply: dronehive-unicode-ci`). `patches --prove` and `--prove-after-apply` use throwaways (never write or reset `/tmp/siblings`). Independent of #8 (branched from merged #3). Expect `siblings.json` array conflict: result should list PRs 2–10. |
