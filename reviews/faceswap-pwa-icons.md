...
Clone: `https://github.com/yuro1991-afk/face-swap-ios.git` → `/tmp/face-swap-ios` (not modified).
HEAD: `bba7188` `feat: Swift shell, service worker, iOS Info.plist`

## Files named in the job

| Path | In tree? | `git ls-files`? |
| --- | --- | --- |
...
`pwa/` has only `index.html`, `manifest.webmanifest`, `sw.js`. No `*.png` in the clone. `.gitignore` does not list those PNGs.
...
`pwa/index.html` line 13 (only icon link; no 192/512):
...
Refs are `/icon-….png` (site root), not `pwa/icon-….png`.

## `make_icons.py` (exists; generator only)

Writes into `pwa/` if Pillow imports (lines 9–18):
...
Missing Pillow: `Pillow not installed; icons are optional. App still runs.` Script was not run here; no generated files.

## Punch list

- [ ] PNG assets are referenced but **not in the repo** at `bba7188`.
- [ ] `make_icons.py` is at repo root only.
