...
test("cli patches lists the catalog", async () => {
  ...
  assert.equal(parsed.nextMissing, "dronehive-seed-work-order-doc-recall-router");
  assert.ok(parsed.missingLaunches > 0);
  assert.equal(
    parsed.preferMissing,
    "node src/cli.js assign --job dronehive-seed-work-order-doc-recall-router --out /tmp/launches",
  );
  ...
  assert.ok(parsed.patches[0].file);
  assert.ok(Buffer.byteLength(chunks.join(""), "utf8") < 80_000);
});
...
test("cli patches --job bloom-ci-lint includes the lint-workflow afterApply", async () => {
  const chunks = [];
  const code = await runCli(["patches", "--job", "bloom-ci-lint"], {
    write: (value) => {
      chunks.push(value);
    },
  });
  assert.equal(code, 0);
  const parsed = JSON.parse(chunks.join(""));
  assert.equal(parsed.patches[0].id, "bloom-ci-lint");
  assert.ok(parsed.applyNext.some((line) => line.includes(".github/workflows/lint.yml") && line.includes("name: lint") && line.includes("npm ci") && line.includes("npm run lint")));
});
